                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($tenants as $tenant)
                        <tr class="tenant-directory-row transition duration-150 hover:bg-indigo-50/40">
                            <td class="px-5 py-4 text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $tenant->tenant_id }}</td>
                            ...
                            <td class="px-5 py-4">
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $statusClasses }}">{{ ucfirst($tenant->status ?? 'active') }}</span>
                            </td>
                            <td class="px-5 py-4 text-slate-600">{{ optional($tenant->plan_started_at)->format('Y-m-d') ?? '—' }}</td>
                            <td class="px-5 py-4 text-slate-600">{{ optional($tenant->plan_ends_at)->format('Y-m-d') ?? '—' }}</td>
                            <td class="px-5 py-4 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    <a href="/tenant/{{ $tenant->tenant_id }}" class="inline-flex items-center gap-1 rounded-xl border border-indigo-200 px-3 py-2 text-xs font-semibold text-indigo-700 transition hover:-translate-y-0.5 hover:border-indigo-300 hover:bg-indigo-600 hover:text-white">
                                        Customize
                                    ...
                                    @if(!empty($tenant->domain))
                                        @php
                                            ...
                                            $tenantUrl = $scheme . '://' . $normalizedDomain . $tenantPort . '/login?force_login=1';
                                        @endphp
                                        <a href="{{ $tenantUrl }}" class="inline-flex items-center gap-1 rounded-xl border border-teal-200 px-3 py-2 text-xs font-semibold text-teal-700 transition hover:-translate-y-0.5 hover:border-teal-300 hover:bg-teal-600 hover:text-white" target="_blank" rel="noopener noreferrer">
                                            Open Tenant
